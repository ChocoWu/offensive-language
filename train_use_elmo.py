    ...
    def __call__(self, train_dataset, valid_dataset, fb_test_dataset, tw_test_dataset):
        ...
        for epoch in range(self.config.epochs):
            ...
            if fb_test_f1 > best_fb_test_f1:
                ...
                visual_confusion_matrix(fb_pred_labels, fb_gold_labels, self.config,
                                        './image/{}_fb_confusion.jpg'.format(time.strftime("%m-%d_%H-%M-%S")))
                visual_confusion_matrix(tw_pred_labels, tw_gold_labels, self.config,
                                        './image/{}_tw_confusion.jpg'.format(time.strftime("%m-%d_%H-%M-%S")))
            ...
            for w_inputs, c_inputs, labels in tqdm(dataset):
                if self.config.use_gpu:
                    ...
                    output, word_weights, sent_weights = self.model(x_word=w_inputs, word_mask=word_mask,
                                                                    sent_mask=sent_mask, x_char=c_inputs, word=None)
